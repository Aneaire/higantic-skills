...
3. Resolve credentials through a named CLI profile by default. If the standalone `@higantic/cli` application is not installed, tell the user to install it separately with the documented npm command, run `higantic setup` to confirm success and review every public skill, then run `higantic auth login` in their own terminal; never ask them to paste the resulting key into a prompt. Each missing skill has its own `[Y/n]` prompt where Enter accepts installation. Interactive login may offer the same missing public catalog after authentication; each installation remains optional. Read `references/auth.md` to confirm the device is authenticated (`higantic auth status`) and to interpret profile, credential, and storage errors. The complete `HIGANTIC_API_BASE_URL` / `HIGANTIC_AGENT_ID` / `HIGANTIC_API_KEY` triple is the CI/noninteractive override: if any one is set, all three are required and the complete triple wins without mixing with profile data. Never request, accept, or pass an API key through a CLI argument, prompt, source file, or committed env file. The live-reference fetch uses no credential.
