...
- **Sharing is opt-in**: HTML publish/pinned-link commands require `html_artifacts:share`; Canvas publishing requires `excalidraw:share`. Publishing requires `--confirm-public-sharing`; deletion requires `--confirm-delete`.
...
- `references/api.md` — full HTML route list, auth/scopes, identity, conflicts, deletion, visibility, sharing, limits, envelopes, and exit codes.
